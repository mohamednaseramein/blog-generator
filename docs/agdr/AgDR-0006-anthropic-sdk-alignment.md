...
> In the context of generating a structured alignment summary from a user's blog brief, facing the need to call an LLM reliably with typed responses, I decided to use `@anthropic-ai/sdk` with `claude-sonnet-4-6` and prompt-engineered JSON output to achieve a fast, structured, and maintainable integration, accepting the trade-off of prompt fragility over the stronger contract of tool-use.
...
| Model | Pros | Cons |
|-------|------|------|
| **`claude-sonnet-4-6` (chosen)** | Best balance of quality, speed, and cost for structured summarisation | Slightly higher cost than Haiku |
| `claude-haiku-4-5` | Faster, cheaper | Lower reasoning quality for nuanced tone/SEO analysis |
...
- Model pinned to `claude-sonnet-4-6` — revisit on next model release.
